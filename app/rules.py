...
def compute_churn_risk(customer: CustomerRequest) -> dict:
    ...
    if len(tickets_30d) > 5:
        return {
            "risk_level": "HIGH",
            "triggered_rule": "More than 5 tickets in last 30 days"
        }

    # Rule 2: HIGH - Month-to-Month contract + complaint ticket
    if (
        customer.contract_type == "Month-to-Month"
        and len(complaint_tickets) > 0
    ):
        return {
            "risk_level": "HIGH",
            "triggered_rule": "Month-to-Month contract with complaint ticket"
        }
    ...
    if charge_increase_pct > 10 and len(tickets_30d) >= 3:
        return {
            "risk_level": "MEDIUM",
            "triggered_rule": "Charge increase with multiple tickets"
        }
    ...
    if len(tickets_7d) > 3:
        return {
            "risk_level": "MEDIUM",
            "triggered_rule": "More than 3 tickets in last 7 days"
        }

    # Default: LOW
    return {
        "risk_level": "LOW",
        "triggered_rule": "No high risk rules triggered"
    }