...
def simulate_ticket_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["charge_change_pct"] = (
        (df["MonthlyCharges"] -
         df["MonthlyCharges"].shift(1).fillna(df["MonthlyCharges"]))
    ...
    df.loc[churn_mask, "ticket_count_30d"] += np.random.randint(
        2, 5, churn_mask.sum()
    )
    df.loc[churn_mask, "complaint_count"] += np.random.randint(
        1, 3, churn_mask.sum()
    )
    df.loc[churn_mask, "sentiment_score"] -= np.random.uniform(
        0.2, 0.5, churn_mask.sum()
    )
